...
def doctor_response(db: Session, profile: DoctorProfile) -> DoctorResponse:
    user = db.get(User, profile.user_id)
    specialty = db.get(Specialty, profile.specialty_id)

    if user is None or specialty is None:
        raise HTTPException(status_code=500, detail="Doctor references invalid data")
...
def get_stats(
    ...
    return AdminStatsResponse(
        ...
        total_logins=db.scalar(select(func.count(LoginHistory.id))) or 0,
        logins_today=db.scalar(
            select(func.count(LoginHistory.id)).where(
                LoginHistory.logged_at >= today
            )
        ) or 0,
    )
...
def list_doctors(
    ...
    return [doctor_response(db, profile) for profile in profiles]
...
def create_doctor(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Doctor could not be created; email or license may already exist",
        )

    db.refresh(profile)
    return doctor_response(db, profile)
...
def update_doctor(
    ...
    if data.is_active is not None and user is not None:
        user.is_active = data.is_active
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Doctor data conflicts with an existing record",
        )
