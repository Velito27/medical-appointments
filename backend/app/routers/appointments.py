...
def create_appointment(
    ...
    doctor = db.scalar(
        ...
        .where(DoctorProfile.id == slot.doctor_id)
        .with_for_update()
    )
    ...
    if active_appointments >= MAX_ACTIVE_APPOINTMENTS_PER_DOCTOR:
        raise HTTPException(
            status_code=409,
            detail="This doctor already has the maximum of 3 active appointments",
        )
